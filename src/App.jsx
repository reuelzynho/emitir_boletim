...
const PlanilhaNotas = () => {
  ...
  // Função para verificar se as notas são válidas
  const validarNotas = () => {
    for (let disciplina in disciplinas) {
      const nota = parseFloat(disciplinas[disciplina]);
  ...
  const exportarParaExcel = () => {
    // Verificando se todas as notas estão válidas
    if (!validarNotas()) {
      setErro('As notas devem estar entre 0 e 10.');
      return; // Impede a exportação caso haja erro nas notas
    }

    setErro(''); // Limpar erro se todas as notas estiverem corretas

    // Se todas as notas estiverem corretas, geramos a planilha
    const dados = [
